...
{
    uint Txrdy = Stat_Reg_Val & STAT_REG_TXRDY_MASK;
    if(Txrdy  == 0)
...
{
    uint Rxrdy = Stat_Reg_Val & STAT_REG_RXRDY_MASK;
    if(Rxrdy  == 0)
...
uint Uart_Get_Stat_ParErr(uint Stat_Reg_Val)
{
    ...
    uint Par_Err = Stat_Reg_Val & STAT_REG_PAR_ERR_MASK;
    if(Par_Err  == 0)
        printf("\nFunc: %s Line: %d: Par_Err is not Set",__func__,__LINE__);
    else
        printf("\nFunc: %s Line: %d: Par_Err is Set",__func__,__LINE__);
...
{
//0x08
   uint Overflow = Stat_Reg_Val & STAT_REG_OVERFLOW_MASK;
    if(Overflow  == 0)
   ...
   uint Uart_Get_Stat_FrmErr(uint Sat_Reg_Val)
{
    //unit frame;
    //Frame ^ Sat_Reg_Val
    uint Frmerr = Sat_Reg_Val & STAT_REG_FRMERR_MASK;
    if(Frmerr  == 0)
