...
#endif
//#if 0
//Assume that the below code as Uart HW IP
   //uint UartHw_ip[24];
   /*uint Reg_Count;
   for(Reg_Count =0; Reg_Count<24; Reg_Count= Reg_Count+4)
	UartHw_ip[Reg_Count] = 0x00; // All reg reset value is set to 0
   UartHw_ip[0x010] = 0x01;  //status register reset value is set to 1
*/
#if 0
UartHw_Ip();
uint *BA;
//printf("\n ba = 0x%x \n",&UartHw_ip[0x00]);
BA = &UartHw_ip[0x00]; //The first Reg address is the base addres
printf("\n BA = 0x%x, val @ BA = 0x%x \n",BA,*BA);
Uart_Set_Tx_Data(BA, 0x00, 0xAC);	
uint Tx_Data = 0;
Tx_Data = Uart_Get_Tx_Data(BA,0x00);
printf("\n Tx Data = 0x%x \n",Tx_Data );
LINE_INFO;
#endif
//#if 0
...
uint *BA;
//printf("\n ba = 0x%x \n",&UartHw_ip[0x00]);
BA = &UartHw_ip[0x04]; //The second Reg address is the base addres
printf("\n BA = 0x%x, val @ BA = 0x%x \n",BA,*BA);
uint Rx_Data = 0;
Rx_Data = Uart_Get_Rx_Data(BA,0x00);
printf("\n Rx Data = 0x%x \n",Rx_Data );
//LINE_INFO;
//#endif
#if 0     
